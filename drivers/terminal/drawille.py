...
#
# adjusted from micropython-lib/collections/defaultdict
#
class defaultdict:
    @staticmethod
    ...
    def rows(self, min_x=None, min_y=None, max_x=None, max_y=None):
        ...
        for rownum in range(minrow, maxrow+1):
            ...
            for x in  range(mincol, maxcol+1):
                ...
                if not char:
                    row.append(chr(braille_char_offset))
                elif type(char) != int:
                    row.append(char)
                else:
                    row.append(chr(braille_char_offset+char))
